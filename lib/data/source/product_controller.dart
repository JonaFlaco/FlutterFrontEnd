...
class ProductController extends StateNotifier<AsyncValue<List<Product>>> {
  ...
  Future<void> addProduct(Product newProduct) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.3.21:3001/api/products'),
  ...
  Future<void> loadProductsFromServer() async {
    try {
      final response =
          await http.get(Uri.parse('http://192.168.3.21:3001/api/products'));
  ...
  Future<void> deleteProduct(int? productId) async {
    try {
      final response = await http.delete(
        Uri.parse('http://192.168.3.21:3001/api/product/$productId'),
        headers: <String, String>{
          'Content-Type': 'application/json',
        },
      );
